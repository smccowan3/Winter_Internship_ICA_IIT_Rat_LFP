...
for i = 1:components %:components-1
    load(['ICA_Output_reorg_trimmed\awakereorg_ICA_' num2str(i) '.mat'])
    awakedata = reorg_ica;
    load(['ICA_Output_reorg_trimmed\anestreorg_ICA_' num2str(i) '.mat'])
    anestdata = reorg_ica;
    ...
    %% plot clr plot
    subplot(3,5,i);
    climtest = freqmeantrial(:);
    CLIM = prctile (climtest, [5 95]);
    imagesc(flipud(freqmeantrial), CLIM)
    title (['IC ' ind])
    xticks(0:100:600)
    yticks(1:1:18)
    colorbar;
 ...
 saveas(gcf, ['Tonotopic_Analysis_trimmed\IC_Cplots_All_Frequencies_timecourse.png'])
...
for i = 1: components
   load(['ICA_Output_reorg_trimmed\awakereorg_ICA_' num2str(i) '.mat'])
    ...
    %% plot clr plot
    subplot(3,5,i)
    climtest = intmeantrial(:);
    CLIM = prctile (climtest, [5 95]);
    imagesc(flipud(intmeantrial),CLIM)
 ...
 saveas(gcf, ['Tonotopic_Analysis_trimmed\IC_Cplots_All_Intensities_timecourse.png'])